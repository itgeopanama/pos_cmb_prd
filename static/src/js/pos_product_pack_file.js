...
models.load_models([
    {
        ...
        fields: ['id','name', 'combo_product_id', 'lst_price', 'is_combo'],
        domain: function(self){ return [['is_combo', '=',true]]; },
    ...
    },
    {
        ...
        fields: ['id','product_id','product_quantity','uom_id','price','name', 'product_template_id'],
...
models.Orderline = models.Orderline.extend({
    getPackProduct:function(pack_product_id,product_price,product_qty){
        ...
        {   
            if(pack_product[i].id==pack_product_id && self.product.is_combo && (pack_product[i].combo_product_id).length>0 )
            {
                for(var j=0;j<(pack_product[i].combo_product_id).length;j++){ 

                    for(var k=0;k<pack_products.length;k++){
                        if(pack_products[k].id==pack_product[i].combo_product_id[j]){
